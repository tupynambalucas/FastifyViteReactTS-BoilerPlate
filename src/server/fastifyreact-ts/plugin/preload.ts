...
// Define a type for the resolved page chunks for clarity
interface PageChunk extends OutputChunk {
  htmlPath: string
  viteMetadata: {
    importedCss: Set<string>
    importedAssets: Set<string> // <-- Add this required property
  }
}

export async function closeBundle (this: PluginContext, resolvedBundle?: OutputBundle) {
  if ((this as any).environment?.name !== 'client') {
    return
  }

  const assetsInlineLimit = this.environment.config.build.assetsInlineLimit ?? 4096
  ...
  const pages = Object.fromEntries(
    Object.entries(resolvedBundle ?? {})
      .filter(([, meta]) => {
        const chunk = meta as OutputChunk
        if (chunk.facadeModuleId?.includes('/pages/')) {
          ;(chunk as PageChunk).htmlPath = chunk.facadeModuleId.replace(/.*pages\/(.*)\.(j|t)sx$/, 'html/$1.html')
          return true
        }
        return false
      })
  ) as Record<string, PageChunk>

  for (const page of Object.values(pages)) {
    const jsImports = page.imports
    const cssImports = page.viteMetadata.importedCss
    const images = (page as any).moduleIds?.filter((img: string) => {
      return ((page as any).modules[img]?.originalLength > assetsInlineLimit) && imageFileRE.test(img)
    }) || []
...
async function appendHead (html: string, ...tags: string[]): Promise<string> {
...
function writeHtml(page: PageChunk, pageHtml: string, distDir: string) {
